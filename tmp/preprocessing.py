...
def preprocess_folder_to_memory(input_dir):
    """
    input_dir 내 모든 이미지를 전처리하여
    (base_name, binarized_image) 튜플 리스트로 반환
    """
    ...
    for img in images:
        ...
        try:
            cropped_color, base_name = detect_and_crop_mask(img_path)
            cropped_bin = binarize_for_ocr(cropped_color)
            results.append((base_name, cropped_bin))
