...
def ocr_images_from_memory(image_list):
    """
    image_list: (base_name, numpy_image) 튜플 리스트
    반환: (base_name, [줄 단위 텍스트]) 튜플 리스트
    """
    results = []
    for base_name, np_img in image_list:
        try:
            ...
            results.append((base_name, lines))
        except Exception as e:
            print(f"OCR 실패: {base_name} - {e}")
